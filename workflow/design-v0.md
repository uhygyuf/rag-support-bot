...
| Escalation only when docs are silent | Prompt rule 7: an explicit human request also calls `create_ticket` | A visitor asking "I want to talk to a human" must reach a person, not be answered from the docs. Verified E2E (exec 29 → ticket tool ran). |
