...
| `IF` → Sheets/Gmail handoff | **Deterministic branch**: `If escalated` (reply contains the fixed handoff sentence) → `Insert Ticket` (Supabase `tickets`) → `Notify Telegram` → `Reply Escalated`; otherwise `Reply Normal` | **2026-09-18 fix.** The agent was asked to call a `toolWorkflow` ticket tool, but on a live run it produced the handoff sentence **without calling the tool** — the ticket was silently lost. Escalation is now a graph structure, not a model decision: the reply text is prescribed verbatim by the prompt and matched by the IF node. `Notify Telegram` carries `onError: continueRegularOutput`, so a notification failure can never break the ticket. |
| Supabase `tableId` | plain string `"tickets"` | **2026-09-18 fix.** The base Supabase node wants a plain string; a resource-locator object (`{__rl:true,…}`) is passed through literally and the API answers `Could not find the table 'public.[object Object]'`. The earlier "ticket created" claim was wrong — nothing had been written. Verified after the fix: the execution data returns the inserted row (`{"id":1,"created_at":…,"question":…}`). |
...
| No greeting / no starting options | `initialMessages` + `agentName`/`agentDescription`/`agentIcon` on the Chat Trigger, and **welcome bubble + teaser + 6 quick-start buttons inside `site/widget.js`** (`data-cta`, `data-welcome`, `data-quick-replies`) | Mirrors the ETS "Anita" pattern the user asked for: greet before opening, then offer one-click topics. **Caveat (n8n 2.38.7):** the hosted chat page renders `initialMessages` but **not** `suggestedPrompts` — only the custom widget shows the buttons; the trigger config is still stored for newer chat surfaces. |
| Escalation only when docs are silent | Prompt rule 3 also escalates an explicit human request (same fixed sentence → same `If escalated` branch) | A visitor asking "I want to talk to a human" must reach a person, not be answered from the docs. Verified E2E (exec 37 → `Insert Ticket` ran and returned the inserted row). |
